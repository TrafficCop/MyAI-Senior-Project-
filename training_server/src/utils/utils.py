...
class DataLoaderCreator():
    ...
    def loadEmailsDataset(self):
        ...
        return train_loader, test_loader, input_shape, output_shape, 'binary_classification', None
    ...
    def loadWeatherDataset(self):
        ...
        classes = list(set(df['weather'].values))
        encoded = encoder.fit_transform(classes)
        class_table = dict(zip(encoded, classes))
        ...
        return train_loader, test_loader, input_shape, output_shape, 'multiclass_classification', class_table


class EvalFns():
    @staticmethod
    def get_all_metrics(model, test_loader, lossFn, task='binary_classification', class_table=None):
        # choose get all multiclass or binary metrics function
        if task == 'binary_classification':
            return EvalFns.get_all_binary_metrics(model, test_loader, lossFn, class_table)
        else:
            return EvalFns.get_all_multiclass_metrics(model, test_loader, lossFn, class_table)

    @staticmethod
    def get_all_multiclass_metrics(model, test_loader, lossFn, class_table):

        def get_class_wise_accuracy(labels, predictions, metrics):
            '''
            Basic function to get class wise accuracies which does not map directly back to initial labels. 
            i.e. returns as x_accuracy (0-n-1) for each of n classes. A better implementation will receive the mappings
            from the encoder, but for now we do not specify
            '''
            totals = {l:(0,0) for l in set(labels)}
            for actual, prediction in zip(labels,predictions):
                correct, total = totals[actual]
                totals[actual] = (correct + (1 if actual==prediction else 0), total + 1)
            
            for label_class in totals.keys():
                metrics[str(class_table[label_class]) + "_accuracy"] = totals[label_class][0] / totals[label_class][1]
        ...
        output_metrics['accuracy_metric'] = float('%.3f' % accuracy)
        ...
        output_metrics = get_class_wise_accuracy(all_labels, all_preds, output_metrics)
        # output_metrics['confusion_matrix'] = confusion_matrix(all_labels, all_preds).tolist()
        return output_metrics

    @staticmethod
    def get_all_binary_metrics(model, test_loader, lossFn, class_table):
        ...
        for real, fake in zip(all_labels, all_preds):
            false_positive += (real == 0) & (fake == 1)
            true_negative += (real == 0) & (fake == 0)
...
class Diagram():
    ...
    def evaluate(self):
        ...
        try:
            return_metrics = EvalFns.get_all_metrics(self.model, self.test_loader, self.loss_fn_string, self.task, self.class_table)
        except Exception as e:
