...
async def process_diagram(data: DiagramRequest):
    ...
    await handle_design_warnings(data.blocks, data.user_id, data.loss_fn)
...
async def handle_design_warnings(blocks, user_id, loss_fn):
    '''
    Sends warnings. Does not interrupt flow of execution
    ''' 

    blocks = sorted(blocks, key=lambda x: x.order)
    update_data = WarningData(job_id=user_id).model_dump()
    ...
    if n_layers == 0:
        ...
        await manager.send_json(user_id, update_data)
